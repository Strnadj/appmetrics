...
function MsSqlProbe() {
  Probe.call(this, 'mssql');
}
util.inherits(MsSqlProbe, Probe);

MsSqlProbe.prototype.attach = function(name, target) {
  var that = this;
  if (name != 'mssql') return target;
 ...
 */
MsSqlProbe.prototype.metricsEnd = function(probeData, method, methodArgs) {
 ...
 */
MsSqlProbe.prototype.requestStart = function(probeData, method, methodArgs) {
  probeData.req = request.startRequest('mssql', 'query', false, probeData.timer);
};

MsSqlProbe.prototype.requestEnd = function(probeData, method, methodArgs) {
  if (probeData && probeData.req) probeData.req.stop({ sql: JSON.stringify(methodArgs[0]) });
};

module.exports = MsSqlProbe;
